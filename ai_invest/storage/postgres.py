...
class PostgresRepo:
    ...
    def fetch_meeting_sessions(self, *, limit: int = 50) -> list[dict[str, Any]]:
        with self.connect() as conn, conn.cursor() as cur:
            cur.execute(
                """
                select meeting_id, meeting_type, status, started_at, ended_at, facilitator, participants, summary, action_items, run_id
                from meeting_sessions
            ...
            rows = cur.fetchall()
        out: list[dict[str, Any]] = []
        for meeting_id, meeting_type, status, started_at, ended_at, facilitator, participants, summary, action_items, run_id in rows:
            out.append(
                {
